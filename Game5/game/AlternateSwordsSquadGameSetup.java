...
public class AlternateSwordsSquadGameSetup implements GameSetup {
	public void setup(List<Knight> knights, List<Ogre> ogres, int nKnights, int nBraveKnights, int nBadOgres) {
		...
		WarriorSquad ogresExpedition = new WarriorSquad("Ogres Expedition");
		...
		for (int i = 0; i < nBadOgres; ++i){
			badOgresSquad.addCombatUnit(new BadOgre(100));
		}
		
		for (int i = nBadOgres; i < numberOfWarriors; ++i){
			commonOgresSquad.addCombatUnit(new Ogre(100));
		}
		ogresExpedition.addCombatUnit(commonOgresSquad);
		ogresExpedition.addCombatUnit(badOgresSquad);

		System.out.println(ogresExpedition.report("\t"));

		ogres.addAll((List<Ogre>) ogresExpedition.getWarriors());
	}
}
